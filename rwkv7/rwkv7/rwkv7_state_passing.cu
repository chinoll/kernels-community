...
void forward_kernel(
    ...
    const i64 s_shift = i64(bb * H + hh) * i64(N * N) + i64(i * N);
    s0_ += s_shift;
    sT_ += s_shift;
...
void rwkv7_state_passing_forward_cuda(
    ...
    forward_kernel<_N_><<<dim3(H, B), dim3(_N_)>>>(
        T,
        H,
        s0,
...
void rwkv7_state_passing_backward_cuda(
    ...
    void *db) {
    assert(T % _CHUNK_LEN_ == 0);
    backward_kernel<_N_><<<dim3(H, B), dim3(_N_)>>>(
        T,
        H,
        static_cast<CBFPtr>(r),
        static_cast<CBFPtr>(w),
